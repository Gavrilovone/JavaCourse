 ...
 * - Extensibility(расширяемость). Где уместно использовать super class там уместно использовать subclass
 ...
 * Допустим есть класс А у которого есть метод work(){ System.out.println("Привет");} и класс В у которого есть метод
 * work(){ System.out.println("Ок"). и класс child С при создании объекта  и вызове метода c.work() - как java поймет 
 * к  методу какого класса он обращается. это одна из причин почему не поддерживается множественное наследование в java 
 */
...
/** 
 * Subclass-ы так же могут расширяться, и в итоге образуется иерархия. Дантист так же является работником.
 * У этой иерархии есть конец. это класс Object. все классы в конечном итоге это прямые или косвенные потомки 
 * класса Object. 

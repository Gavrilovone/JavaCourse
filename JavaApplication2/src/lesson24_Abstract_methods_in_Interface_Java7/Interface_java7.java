 ...
 * является классом 
 * Не возможно создать объект интерфейса, потому что это не класс
 * У интерфейса нет конструкторов
 * Access modifier, правило, что  одиом пакете может быть один public,  и имя дожно совпадать с именем файла действует
 * и для интерфейса 
 * интерфейс не может быть final и методы интерфейса не могут быть final 
 * из переменных в методе нтерфейса может быть только константы
 * Все что есть в классе, есть в его sub классе и все что есть в интерфейсе есть  в классе, который делает его implement
 */

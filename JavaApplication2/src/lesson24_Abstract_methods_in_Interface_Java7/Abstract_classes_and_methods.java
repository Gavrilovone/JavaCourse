 ...
 * final - потому что final не может быть перезаписан, а абстрактный метод должен быть перезаписан
 * private - по тойже причине, потому что он не виден в sub классе и не может быть перезаписан
 * static - потому что статические методы не могут быть перезаписаны так же
 * 
 * Класс который перезаписывает все методы абстрактного класса называется конкретныйм классом, у него нет абстрактных методов
 * если дочерний класс перезаписал все методы абстрактного класса, то его наследник может уже их не презаписывать
 */
